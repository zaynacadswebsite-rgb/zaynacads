...
const skillsData = {
  writing: [
    { label: 'Chapter 1–3 Writing', bg: '#2c3e50', text: '📖' },
    { label: 'Chapter 4–5 Writing', bg: '#34495e', text: '📝' },
    { label: 'Research Design', bg: '#0f172a', text: '🧭' },
    { label: 'Literature Review', bg: '#8b5cf6', text: '📚' },
    { label: 'Title Defense Prep', bg: '#111827', text: '🎤' },
    { label: 'Concept Papers', bg: '#0ea5e9', text: '💡' },
  ],
  stats: [
    { label: 'Excel Data Analysis', bg: '#1d6f42', text: '📊' },
    { label: 'Survey Design', bg: '#336791', text: '📋' },
    { label: 'Hypothesis Testing', bg: '#c53030', text: '🧪' },
    { label: 'Results Interpretation', bg: '#111827', text: '📈' },
    { label: 'SPSS', bg: '#2c3e50', text: 'S', hidden: true },
  ],
  support: [
    { label: 'Turnitin Checker', bg: '#ffffff', img: 'icons/turnitin-logo.png' },
    { label: 'Grammarly', bg: '#ffffff', img: 'icons/grammar-logo.png' },
    { label: 'APA / MLA Formatting', bg: '#111827', text: '📐' },
    { label: 'Plagiarism Checking', bg: '#2496ed', text: '🔍' },
    { label: 'Slide Deck Prep', bg: '#a259ff', text: '🖥️' },
    { label: 'Mock Defense Coaching', bg: '#007acc', text: '🗣️' },
    { label: 'Reference Management', bg: '#0d1117', text: '🔗' },
  ]
};
...
function renderSkills(tab){
  skillsGrid.innerHTML = skillsData[tab].map((s, i) => `
    <div class="skill-item pop-in" style="animation-delay:${i * 45}ms; ${s.hidden ? 'visibility:hidden;' : ''}">
      <div class="icon-box" style="background:${s.bg}; color:${s.dark ? '#1a2332' : '#fff'};">
        ${s.img
          ? `<img src="${s.img}" alt="${s.label}" style="width:60%; height:60%; object-fit:contain;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><span style="display:none; align-items:center; justify-content:center; width:100%; height:100%; font-size:0.7rem; font-weight:700;">${s.label.split(' ').map(w => w[0]).join('').slice(0,2)}</span>`
          : s.text}
      </div>
      <span>${s.label}</span>
    </div>
...
// Playlist — add/remove filenames here as needed.
const playlist = [
  'music/song1.mp3',
  'music/song2.mp3',
  'music/song3.mp3',
...
// Pick a random track, avoiding an immediate repeat when advancing.
let lastTrack = null;
...
// Not looping a single song anymore — when one ends, shuffle to another.
bgMusic.loop = false;
